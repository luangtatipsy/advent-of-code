...
def fold(points: Set[Tuple[int, ...]], instructions: List[Dict[str, int]]) -> int:
    instruction = instructions[0]
    for axis, fold_pos in instruction.items():
        if axis == "x":
            points = set(
                (x if x < fold_pos else fold_pos - (x - fold_pos), y) for x, y in points
            )
        elif axis == "y":
            points = set(
                (x, y if y < fold_pos else fold_pos - (y - fold_pos)) for x, y in points
            )
        else:
